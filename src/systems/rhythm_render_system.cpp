...
public:	
	void render(RenderContext& rc)
    {
		for (auto& c: cameraFamily) {
			auto cam = Camera().setPosition(c.transform2D.getLocalPosition()).setZoom(c.camera.zoom);
...
private:
	void paint(Painter& painter)
	{
		for (auto& e: mainFamily) {
			...
			auto col2 = col;
			col2.a = 0.35f;
			painter.drawCircle(e.transform2D.getLocalPosition(), endRadius, 2, col2);
			painter.drawCircle(e.transform2D.getLocalPosition(), radius, 2, col);
		}
	}
};
